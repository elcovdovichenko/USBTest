// Main program for usbprobe driver
...
NTSTATUS AddDevice(IN PDRIVER_OBJECT DriverObject, IN PDEVICE_OBJECT pdo)
	...
	_snwprintf(namebuf, arraysize(namebuf), L"\\DosDevices\\USBPROBE_%d", InterlockedIncrement(&devcount));
	...
	KdPrint((DRIVERNAME " - Device Name is: %ws\n", devname.Buffer));
	...
	__try
        ...
        status = IoRegisterDeviceInterface (pdo, 
           (LPGUID) &GUID_USBEXAMINATION_INTERFACE_CLASS, NULL, &pdx->InterfaceName);
		if (!NT_SUCCESS(status))
			{						// can't register device interface		
			KdPrint((DRIVERNAME " - IoRegisterDeviceInterface failed - %X\n", status));
			__leave;
			}						// can't register device interface
        KdPrint((DRIVERNAME " - IoRegisterDeviceInterface status = %X\n", status));
...
VOID EnableAllInterfaces(PDEVICE_EXTENSION pdx, BOOLEAN enable)
	{							// EnableAllInterfaces
	IoSetDeviceInterfaceState(&pdx->InterfaceName, enable);
	}							// EnableAllInterfaces
